...
**7/15時点**：TrackEvalのCLI/API parityを1および3 sequenceで確認し、Mamba側ではtracker推論 subprocessを維持したままTrackEval評価を直接関数呼び出しへ置換した。既存25 sequence tracker出力でCLIとadapterのsummaryが一致した。学習ループ実行と計算時間の比較は未実施。
...
| 日付 | 内容 |
|------|--------|
| 2026-07-15 | TrackEval CLI/API parityとMamba側評価adapterのfull-val parityを確認。学習ループは未変更。 |
