...
status: active
summary: TrackEvalのCLI/API parityとMamba側の評価adapter統合を確認済み。学習ループは未変更で、次は計算時間分析と実学習smokeを進める。
created: 2026-07-07
...
**7/15時点**：TrackEvalのCLI/API parityを1および3 sequenceで確認し、Mamba側ではtracker推論 subprocessを維持したままTrackEval評価を直接関数呼び出しへ置換した。既存25 sequence tracker出力でCLIとadapterのsummaryが一致した。計算時間profilingの計測コードを追加し、1epoch tiny smokeでtrain/val loss/epoch totalのログ取得まで確認した。tracker推論・TrackEvalの実時間比較と100epoch本学習は未実施。
...
| 日付 | 内容 |
|------|--------|
| 2026-07-15 | TrackEval CLI/API parityとMamba側評価adapterのfull-val parityを確認。計算時間profiling計測を追加し、1epoch tiny smokeを確認。 |
