...
研究相談、brainstorm、MTG後TODOから派生した外部実装リポジトリの変更または実験設計変更を始める前に、以下を確認してください。
...
いずれかが欠ける場合は、外部実装リポジトリを編集せず、`brainstorm` による計画化または不足確認へ戻してください。Plan modeが利用できない場合も、Plan mode相当の計画化を行い、このゲートを省略しないでください。

次は本ゲートの対象外です。

- 読み取り専用の調査・診断
- ダッシュボード、TODO、議事録、brainstormメモなどのResearch Workspace管理
- 文書表現のみの軽微な修正

ユーザーがspec省略を明示する場合は、例外にする理由と影響を短く確認してから進めてください。
